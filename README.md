...
1. A patient cannot register online for two appointments with the same doctor within 24 hours.
...
Timegate.Check(id string, // Event ID
    ...
    before time.Duration, // Period of time before the timestamp
    after time.Duration, // Period of time after the timestamp
...
1. Thus, a patient cannot register for a second appointment at 10:00 AM the following day ; with already another appointment at 7:00 PM the day before.

Nor can the patient register for another appointment at 4 PM the same day.
...
24 hours after the consultation time, the 7 PM lock gets removed, and it becomes possible to reschedule.

2. To reserve a tennis court at 4 PM for 1 hour, valid for a maximum of 15 minutes after the start of the time slot, the parameters are:
...
Otherwise, another player may request to use the court for the remaining time of the time slot:
...
Thus, if the vehicle enters and exits the parking lot several times that day, it will only be billed once, for the entire day, even though the events corresponding to its repeated entries and exits may have been communicated to the processing system with some delay, but within the 6-month limit.

Technically, the Timegate module relies on an etcd database to benefit — if needed — from distributed lock management within a cluster such as Kubernetes ; with processing workflows deployed as containers.
